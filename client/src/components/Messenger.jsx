...
const Messenger = () => {
  ...
  const handleSearch = async () => {
    const res = await axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:4000"}/users/search?query=${query}`);
    setUsers(res.data);
  };

  const selectUser = async (user) => {
    setSelectedUser(user);
    const res = await axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:4000"}/api/messages/${loggedInUser._id}/${user._id}`);
    setMessages(res.data);
  };

  const sendMessage = async () => {
    if (!text) return;
    const res = await axios.post(`${import.meta.env.VITE_API_URL || "http://localhost:4000"}/api/messages`, {
