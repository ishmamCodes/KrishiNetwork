...
const OrderList = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:4000"}/orders/all`);
  ...
  const handleMarkAsDelivered = async (orderId) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:4000"}/orders/${orderId}/status`, {
        method: 'PUT',
