...
const Messenger = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:4000"}/users/search?query=${searchQuery}`);
  ...
  const fetchMessages = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:4000"}/api/messages/${loggedInUser.phone}/${selectedUser.phone}`);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL || "http://localhost:4000"}/api/messages`, payload);
      setText('');
      fetchMessages();
    } catch (error) {
